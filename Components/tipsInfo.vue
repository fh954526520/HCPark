	...
	<cover-view class="content">
		<cover-view class="topsinfo-content">
			...
			<cover-view class="plate-number">
				<cover-view :key="index" :style="{'background-color':index==7 ? '#E4FFEA':'','border-color':index==7 ? '#7BC623':''}" class="plate-items" v-for="(item,index) in 8">
					    {{platenumber[index]}}
				</cover-view>
			</cover-view>
			
			<cover-view class="changenum">
				<cover-view style="color:#787878; border-bottom: 1px solid #787878;font-size: 26rpx;">
					更换你的车牌号码?
				</cover-view>
			</cover-view>
			
			<cover-view class="btn" @click="confirm">
...
<script>
	export default{
		...
		methods:{
			confirm:function(){
				uni.navigateTo({
					url:"/pages/payfortips/payfortips?platenumber="+this.platenumber,
				})
			}
		}
	}
